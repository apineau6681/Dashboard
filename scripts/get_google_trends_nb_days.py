...
# Step 2: Fetch Google Trends data and store in 'btc_trend'
pytrends = TrendReq(hl='en-US', tz=360)
pytrends.build_payload(['Bitcoin'], timeframe=f'{trend_start_date.strftime("%Y-%m-%d")} {trend_end_date.strftime("%Y-%m-%d")}', geo='', gprop='')
trends_df = pytrends.interest_over_time().drop(columns=['isPartial'], errors='ignore').reset_index()
trends_df.rename(columns={'date': 'date', 'Bitcoin': 'BTC_trend'}, inplace=True)
btc_trend_worksheet = output_spreadsheet.worksheet(BTC_TREND_SHEET)
set_with_dataframe(btc_trend_worksheet, trends_df)
print("BTC trend data inserted into btc_trend tab.")
